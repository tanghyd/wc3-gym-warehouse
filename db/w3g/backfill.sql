...
-- LIMIT 1 BY replay_id keeps one glob from staging the same replay twice.
-- The views.sql cascade fans every new row out to replay_events on its own.
--
-- Invoke:
--   just local::backfill 'http://localhost:9000/warehouse/parsed/v2/**.json'
--   just local::backfill 'https://<account>.r2.cloudflarestorage.com/<bucket>/parsed/v2/dt=2026-09-*/**.json'
--
