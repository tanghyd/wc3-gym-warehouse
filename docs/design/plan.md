...
- Data state: the local ClickHouse 26.9.1.1204 holds 6,567 replays, 13,134 `replay_players` rows and 950,132 `replay_events` rows (ingested 16:47:52 to 17:27:57, `replays.ingested_at`). This is the "full load" base of stories.md D3.
...
- "(26.9)" marks a number measured on 26.9.1 before the repeat flag. PR 2 re-measures it on 26.8 with the flag and writes it into the document that owns it.
...
| Result parity | Per story Review case, `POST /search` returns the replay-id set of the frozen page SQL, on the fixtures and the full load | `just api-parity` (rust.md 17.3) |
| cargo | tests pass with `--locked`; clippy `-D warnings`; `cargo fmt --check` | `just test`, `just api-lint` |
| EXPLAIN | `EXPLAIN indexes = 1` shows the `Keys`, the `Condition` and a `Granules` count no worse than the PR's table (`schema-pk-filter-on-orderby`) | `clickhouse-client` with the golden params |
| Fixture numbers | Every stories.md fixture number matches exactly (stories.md D3) | the fixture server (PR 1) |
| Full-load numbers | Re-measured, dated, written into the PR. They inform; they do not fail a PR. | the local server |
| node test, vite build | `npm test` (`query.test.mjs`) and `npm run build` pass | `npm --prefix frontend test`, `just ui-build` |
| Playwright shots | 1440 px and 390 px, light only, every state of the page. No horizontal page scroll. The author names what each shot checks. Shots go into the PR as an artifact. | the `design-shots/` harness pattern |
...
Needed: stories.md D3 needs a fixture base. `just fixtures` does not exist (justfile:1-68).

| | |
|---|---|
| Files | `justfile`: `fixtures` (rust.md §16, refreshing `w3g.refresh__opener_rollup`), `load-local <glob>` (the `replay_id` gate of backfill.sql:22-23, `file()` from the user-files path, default glob `data/parsed/gnl/`), `fixture-server`. `db/w3g/backfill.sql`: `LIMIT 1 BY replay_id` (queries.md §5). `infrastructure/local/`: a second config with its own `STATE_DIR` and ports. |
| Gates | Fixture server: `replays FINAL` 3, `replay_players FINAL` 6, `replay_events` 323. A second `just fixtures` adds no row. `just fixtures` checks `input()` + `JSONAsString` and waits for `SYSTEM REFRESH VIEW` to finish. `just load-local` twice: `count()` = `count() FINAL` on `replay_events`, 0 key groups with more than 1 row (`insert-optimize-avoid-final`). A doc twice in one glob stages once. `file()` reads the glob in one `INSERT` (`insert-batch-size`). |
...
| | |
|---|---|
| Files | `justfile`: `battle-test <dir>` (rust.md §16). It sets every S3 variable (`W3WAREHOUSE_S3_PREFIX=dev`, MinIO on 9100), runs `minio.sh up`, uploads each `<stem>.w3g` as `dev/replays/<stem>/game1.w3g` with the `mc` that `minio.sh` caches, then `just drain-once` and `just backfill-bucket`. No new script. |
| Steps | On an empty server, so the 26.9 dev load stays as the oracle: `just fixture-server battle`; `CLICKHOUSE_PORT=9004 just schema mappings`; `CLICKHOUSE_PORT=9004 just battle-test <main clone>/data/deploy/gnl`. The stems are numeric (13185269 to 13738541, in u32), so `parse_key` (drain.rs:113-120) accepts them. |
| Gates | `replays FINAL` equals the `file()` load of the same set: 6,564. A second `just drain-once` parses 0 files. Every failure is listed with its reason. The `dev/` prefix and its fake series ids never reach the org deploy. |
...
| Files | `db/w3g/tables.sql` (key at :213, comment :195-197; drop :237-254; add `openers`; `is_repeat UInt8 DEFAULT 0` on `player_order_events` and `replay_events`, `schema-types-minimize-bitwidth`, `schema-types-avoid-nullable`). `db/w3g/views.sql` (compute `is_repeat` in the order MVs, `mv__order_unknown` included, carry it through `mv_events__order`; add `refresh__openers`; drop :433-470). `justfile` (:47 and the `fixtures` refresh become `w3g.refresh__openers`). `compose.yaml:12`, `:43`, `infrastructure/ci.yml:26` to `26.8` (today 24.10). |
...
| How it lands | ClickHouse is derived state (PLAN.md:105): `DROP DATABASE w3g`, `just schema mappings`, `just load-local`, `just fixtures`. No `EXCHANGE`, no hand `DROP`. The box needs the same rebuild if live. |
...
| 26.8 re-run | The local server is 26.9.1. Take the 26.8.2 single binary from the official ClickHouse GitHub release; run it with its own `STATE_DIR` and ports; load the full load and fixtures; re-run every EXPLAIN in queries.md §5 and §7 and rust.md §11 M1-M7, M11-M14. The runtime join filter needs 26.2+. |
...
| Files | `infrastructure/docker/clickhouse/api-user.xml`, `compose.yaml` (volume, env, `chapi`), `infrastructure/local/server.sh` (copy into `config/users.d/`), `.env.example` (`CLICKHOUSE_API_PASSWORD=`), `.dockerignore` (`**/target/`) |
| Gates | As `warehouse_api` over HTTP on 26.8, host and compose: `SELECT {x:UInt8}` with `param_x` works; a `SETTINGS` clause gives 164; over `max_rows_to_read` gives 158; a login from the `default` network gives 516, with the right and with an empty password (rust.md §18); `SELECT` outside `w3g` is denied; the host server merges `users.d` (`agent-query-safety`). |
| Daniel reviews | Subnet `172.30.87.0/24` against the box's Docker networks. Set the password on the box. |
...
| | |
|---|---|
| Scope | The Vite frame: shell, router, API client, URL codec, the light tokens (frontend.md §6, `design/mockups/tokens.css`), copied gnl components, `FilterRow`, `StateBlock`, `ReplayTable`, `ObjectIcon`, `NotFoundView`. nginx serving and the tunnel move (rust.md 8.1). |
...
| Gates | node test, build. The dataviz validator on the chart pairs, light surface (frontend.md 6.5). Shots of the shell and `/nope`. `curl /api/health` through nginx gives 200. |
...
| | |
|---|---|
| Scope | `StatsView`, `ColumnChart`, `BarList`, `DivergingBars`, `ChartTooltip`, `useWidth` |
| Gates | Shots of chart and table views. Fixture numbers through the UI. Keyboard focus opens each tooltip. Chart pairs pass the validator on the light surface. |
...
- P1: the fixture base runs on a second local server (`just fixture-server`; `CLICKHOUSE_URL` points the API at it). Why: a Review runs in a browser; CI has none.
...
ClickHouse
- Pin 26.8 (LTS); PR 2 re-runs the 26.9 checks on it.
...
Frontend
- Style: GNL stone-and-bronze tokens, light only (frontend.md §6). No dark theme, no theme menu.
...
| When to design a dark theme | With the GNL theme PR, or later | With the GNL theme PR: one shared dark palette | GNL theme PR |
| Final win/loss pair | Blue `#2A6496` and red `#B5452F`, or gnl's green and red | Blue and red: PASS all in frontend.md 6.5 | GNL theme PR |
