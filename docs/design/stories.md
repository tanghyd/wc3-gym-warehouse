...
| Race display | Race ids on the wire are the GNL ids `HU`, `OC`, `NE`, `UD`, `RANDOM` (gnl `app/models/enums.py:4-9`); `RANDOM` is a fifth race. Storage keeps the parser's letters `H O N U R`, and the Rust API maps between the two at its boundary (queries.md 3.7). An unknown id gives 400 `groups[0].race: must be one of HU, OC, NE, UD, RANDOM`. The frontend uses the GNL ids everywhere. Player race uses the `RaceSelect` and `RaceIcon` pattern from wc3-gym-frontend. The icon shows only when the row has a race (PlayerName.vue `v-if="race"`). A player shows `{name} {race}`: no flag, no MMR until the dims loader. |
...
| Empty state | Says what to change, e.g. "No replay matches. Drop a step or widen the scope." (index.html:289). |
